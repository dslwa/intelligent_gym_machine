"""
Główny silnik systemu wnioskowania rozmytego (FIS) typu Mamdani.
Klasa IntelligentGymMachine implementuje sterowanie maszyną treningową.
"""
...
class IntelligentGymMachine:
    """
    Klasa implementująca inteligentny system sterowania maszyną treningową
    oparty na logice rozmytej (Fuzzy Inference System - Mamdani).
    """

    def __init__(self):
        """Inicjalizacja systemu FIS."""
    ...
    def setup_variables(self):
        """Definicja zmiennych lingwistycznych (wejściowych i wyjściowych)."""
    ...
    def setup_membership_functions(self):
        """Definicja funkcji przynależności dla wszystkich zmiennych."""

        # 1. SIŁA GENEROWANA [0-500 N]
        ...
        # 7. SYGNAŁ FEEDBACKU [1-5] - WYJŚCIE
    ...
    def setup_rules(self):
        """
        Definicja bazy reguł rozmytych.
        Reguły oparte na wiedzy eksperckiej z zakresu biomechaniki
        i fizjologii treningu siłowego.
        """
        self.rules = []

        # REGUŁY DLA ACCOMMODATING RESISTANCE (R1-R6)
        ...
        # REGUŁY ZWIĄZANE Z PRĘDKOŚCIĄ RUCHU (R7-R11)
        self.rules.append(ctrl.Rule(
        ...
        # REGUŁY ZWIĄZANE ZE ZMĘCZENIEM (R12-R16)
        self.rules.append(ctrl.Rule(
        ...
        # REGUŁY DLA TRYBÓW TRENINGOWYCH (R17-R22)
        self.rules.append(ctrl.Rule(
        ...
        # REGUŁY OPTYMALIZACYJNE (R26-R30)
        self.rules.append(ctrl.Rule(
    ...
    def compute(self, sila_val, predkosc_val, faza_val, zmeczenie_val, tryb_val):
        """
        Obliczenie wartości wyjściowych dla zadanych wartości wejściowych.

        Parameters:
        -----------
        sila_val : float - Siła generowana [N], zakres 0-500
        predkosc_val : float - Prędkość ruchu [m/s], zakres 0-1.5
        faza_val : float - Faza ruchu [% ROM], zakres 0-100
        zmeczenie_val : float - Wskaźnik zmęczenia [%], zakres 0-100
        tryb_val : float - Tryb treningu (1=siłowy, 2=hipertrofia, 3=wytrzymałość)

        Returns:
        --------
        dict : słownik z wartościami oporu i feedbacku
        """
    ...
    def _get_feedback_text(self, feedback_val):
        """Konwersja wartości liczbowej feedbacku na tekst."""
        if feedback_val < 1.5:
            return "ZWOLNIJ"
    ...
    def get_membership_functions_table(self):
        """
        Generuje tabelaryczne zestawienie parametrów funkcji przynależności.
        """
        tables = []
    ...
    def get_rules_documentation(self):
        """
        Generuje dokumentację reguł w formacie IF-THEN z interpretacją semantyczną.
        """
        rules_doc = """
================================================================================
                    ROZMYTA BAZA WIEDZY - REGUŁY IF-THEN
================================================================================

A. ŹRÓDŁO WIEDZY EKSPERCKIEJ
----------------------------
Reguły zostały opracowane na podstawie:
1. Literatury z zakresu biomechaniki i fizjologii treningu siłowego:
   ...
   - Kompf, J., Arandjelović, O. (2016). "Understanding and Overcoming the Sticking Point"

2. Wiedzy eksperckiej z zakresu treningu siłowego:
   - Krzywa siły (strength curve) - zmiana momentu siły w zależności od kąta stawowego
   - Zjawisko "sticking point" - punkt najsłabszej pozycji mechanicznej
   - Accommodating resistance - dopasowanie oporu do krzywej siły

B. UZASADNIENIE OPERATORÓW LOGICZNYCH
-------------------------------------
* AND (T-norma): MINIMUM - konserwatywne podejście, wynik ograniczony przez najsłabszy warunek
* OR (S-norma): MAXIMUM - aktywacja gdy spełniony jest przynajmniej jeden warunek
* IMPLIKACJA: MINIMUM (Mamdani) - zachowuje interpretowalność reguł
* AGREGACJA: MAXIMUM - zachowanie wpływu wszystkich aktywowanych reguł
* DEFUZYFIKACJA: CENTROID - gładkie, ciągłe wyjście (bezpieczeństwo)
